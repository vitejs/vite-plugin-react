...
[React Compiler](https://react.dev/learn/react-compiler) support is available via the `compiler` option, which requires [`oxc-transform-react`](https://npmx.dev/package/oxc-transform-react) as an optional peer dependency:
