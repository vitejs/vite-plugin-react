...
This example demonstrates a minimal framework-owned cache for functions and components that execute inside the RSC environment. It composes `@vitejs/plugin-rsc`'s generic inline directive transform with its low-level argument and Flight serialization APIs.
...
```text
inline cached function
  -> hoisted implementation + direct closure arguments
  -> cache wrapper
  -> RSC-serialized argument key
  -> cached Flight result
```

The generated implementation stays module-local. This example therefore concentrates on cache-runtime composition without server-reference registration, client proxies, form transport, or capture encryption.

## Runtime semantics

The cache wrapper uses `encodeReply()` to serialize call arguments into the same protocol React uses for Server Function arguments. A promise-valued entry deduplicates concurrent misses for each serialized key. On a miss, the runtime reconstructs the arguments with `decodeReply()`, invokes the implementation, and serializes its result with `renderToReadableStream()`.

Results are retained as Flight streams rather than resolved JavaScript values. `StreamCacher` tees the stored stream for every read, and `createFromReadableStream()` decodes each branch with the temporary-reference set from that invocation. This preserves RSC serialization semantics when cached output contains React nodes or references.

String-encoded replies can be used directly as keys. Multipart replies are hashed entry by entry because serializing an entire `FormData` response would introduce a random boundary and make equal arguments produce different keys.
