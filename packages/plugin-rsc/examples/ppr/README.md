...
In Next.js, separate passes are required because the initial pass omits staged rendering, dynamic tracking, and vary-parameter accumulation. The final pass enables the semantics used to produce the authoritative artifact. vinext similarly gives its warmup phase a cache-discovery-only contract, then resets PPR state before the final render. A framework needs separate passes when cache discovery runs without metadata, tracking, staging, or cutoff semantics required by the persisted result. Separate passes are not required merely because cache fills were initially pending.
