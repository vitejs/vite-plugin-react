...
The framework plugin calls plugin-RSC's experimental `manager.getClientReferenceServerReferences(this)` method during its final client `generateBundle` hook and writes the result to `dist/client/reference-reachability.json`. It demonstrates that final client module-graph traversal connects `client-form.jsx` to `objectWrappedAction` without parsing local JavaScript value flow.
