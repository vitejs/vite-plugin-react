...
function defineTest(f: Fixture) {
  ...
  testNoJs('css @nojs', async ({ page }) => {
    await page.goto(f.url())
    await testCss(page)
    // Only check stylesheets generated by plugin-rsc, not user-authored links.
    ...
    // Rendered links preserve `"anonymous"`, while ReactDOM.preinit normalizes it to `""`.
    expect(
      crossOrigins.every((value) => value === 'anonymous' || value === ''),
    ).toBe(true)
  })
