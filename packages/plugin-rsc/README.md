...
- [`./examples/use-cache-persistent`](./examples/use-cache-persistent) - Persistent callable cache entries demonstrating Flight value replay and dependency-aware development invalidation.
...
graph TD
    ...
    subgraph "<strong>rsc environment</strong>"
        A["React virtual dom tree"] --> |"[@vitejs/plugin-rsc/rsc/server]<br /><code>renderToReadableStream</code>"| B1["RSC Stream"];
    end
...
Prefer the explicit `/rsc/server` and `/rsc/client` entry points when only one protocol side is needed. Besides making API ownership explicit, this allows Vite to avoid loading and bundling the unused `react-server-dom` runtime. Use `/rsc` when both sides are intentionally needed.
...
- `@vitejs/plugin-rsc/rsc/server`, `@vitejs/plugin-rsc/rsc/client`, `@vitejs/plugin-rsc/rsc/static`, `@vitejs/plugin-rsc/ssr`, and `@vitejs/plugin-rsc/browser` are the recommended portable entry points for application and framework runtime code. Their `.node` variants described above are the recommended entry points for native Node.js streams. They initialize the plugin's built-in module loading using the manifests generated by Vite and re-export the corresponding React runtime APIs.
- `@vitejs/plugin-rsc/react/rsc`, `@vitejs/plugin-rsc/react/ssr`, and `@vitejs/plugin-rsc/react/browser` are low-level portable runtime adapters. Native Node.js stream adapters are available from `@vitejs/plugin-rsc/react/rsc/server.node`, `@vitejs/plugin-rsc/react/rsc/client.node`, `@vitejs/plugin-rsc/react/rsc/static.node`, and `@vitejs/plugin-rsc/react/ssr.node`. They are used by code generated for `"use client"` and `"use server"`, and by custom integrations that provide module loading through `setRequireModule` instead of using the plugin's generated manifests.
